...
Social-ecological systems can show non-linear and surprising behaviours in response to changes in social and environmental conditions @reyersSocialecologicalSystemsInsights2018. For example, tipping points - where there is rapid change in system state when some threshold is passed - and hysteresis - the difficulties in reversing these changes - challenge xxx [ref!].  The ability of a system to resist such change is often refereed to as its 'resilience' [ref!].    


Social-ecological memory (SEM) is a fundamental component of a social-ecological system's resilience to change @folkeSynthesisBuildingResilience2002.  It is dynamic and continually reshaped in positive and negative ways, both internally (e.g., as social norms within a group change) and externally (e.g., as knowledge from beyond a group is transferred into it).  Knowledge-belief-practice (KBP) complexes underpin SEM. representing the long-term set of behaviours developed by humans based on repeated interactions with their environments [ref!]. A repeated outcome of colonialism has been the erosion and transformation of KBP complexes, whether intentional or otherwise, such that they become quiescent. For example, many indigenous people have been deprived of access to resources by legislative means (a direct loss) and through dwindling populations of some species (an indirect loss); of course, these losses interact as perceived conservation threats leads to regulatory mechanisms aimed at 'protecting' a species #cite(<hartelTraditionalEcologicalKnowledge2023>).  There is growing interest in understanding the conditions under which KBP are maintained, the trajectory of their quiescence or loss, and how they might be reinstated.
...
Abrams and Strogatz #cite(<abramsModellingDynamicsLanguage2003>) present a minimal model of language change between speakeRs of two languages (X and Y), which is aggregated and not spatial but informative:
...
Analysis of Abrams and Strogatz's #cite(<abramsModellingDynamicsLanguage2003>) model suggests that a truly bilingual society with mixed populations of speakers is unstable.  However, they also show that the status ($s$) of a language (i.e., the opportunities its users derive) is critical in its persistence; hence, Abrams and Strogatz #cite(<abramsModellingDynamicsLanguage2003>) argue, education and other policies have a critical role in a language's persistence. In a spatial agent-based model informed by XYZ, Civico [ref!] explored how speakers might shift between competing languages and the implications for these dynamics of the persistence of minority language.  The model outcomes suggest that exogamy negtaively affects persistence, especially via effects on inter-genenerational transmission, but that active education of the minority language has strong and persistent effects, especially if initiated prior to significant decline. Thus, these models point to the importance of knowledge transmission, and benefits and opportunity for use as key determinants of the persistence of minority languages.
...
1. How do different modes of knowledge exchange affect levels of knowledge about the use of a resource in a stable environment?    

2. How do KBP respond to socio-ecological change (loss and reinstatement of access):
...
Here we provide a high-level description of the agent-based model we used to explore the conditions under which biocultural hysteresis might arise; a full description using the ODD protocol (Grimm ref!) is provided as Supplementary Material. Our ABM is implemented in NetLogo 7.0.3 #cite(<wilenskyNetLogo6401999>).


== Model description

==== Landscape
The model landscape is a grid with two types of grid cells (labelled 'a' and 'b') representing sources of different knowledge - these represent places where specific species or geographic features are present. The landscape is  a 50 x 50 square lattice of cells (patches). We assume that a patch can not contain both types of resource. The types are not inherently positive or negative, just different.  The initial amount of type 'a' in the landscape is controlled by the `n-p-a` parameter, with patches allocated to each type at random.    
...
2. movement can be at random, or agents can preferentially move to a neighbouring cell with the type they are most knowledgeable about; in the second case we also apply a multiplier representing an inherent preference for one of the two types (set at 1.0 unless otherwise stated)     
...
2. Questions about proximity and exchange Effects of different transfer types

2. Effects of different proximities
- scenario 1 with cognitive distance (10, 20, 30, 60) and credibility threshold (on-off)


==== Amount and rate of loss scenarios

We explored the implications of the loss of access/availability of one of the resource types (type 'a') to assess how different modes of learning influence the retention of knowledge.  We simulate this loss by changing patches from one type to another at a specified rate; this could represent the loss of a species or the loss of access to the species (e.g., via protectionist regulations).   For each of the eight learning combinations explored under baseline conditions we evaluated four different amounts of loss that results in a decline to 80%, 60%, 20% and 0% of the initial resource availability in the landscape.  The loss of knowledge occurred over a 250 time-step period. We also evaluated explored the rate of loss of access to the resource, holding the total amount constant, again varying the modes of learning.  We evaluated four rates of loss equating to a loss of c. 60% of the initial resource availability. IFor both amoutn and rate of loss we evaluated scenarios where the loss was permanent or where it returned at the same rate to the initial conditions.


==== Amount and rate of loss scenarios
We explored the effects of an asymmetriuc preference for the focal resource ('a') to assess the extent to which it might counter any hysteresis effects -- this is analogous to the status effect described by ref!.  to acheive this we explored a scenario where c. 60% of access was lost with a preference for resource 'a' over 'b' in the interval (1.0, 1.25, 0.05).
...
We analysed the data visually and did not use frequentist statistics, following #cite(<whiteEcologistsShouldNot2014>, form: "prose"). We used R version 4.5.2 for the analyses with packages XYZ.
...
- is the word quiescent better (MF)
